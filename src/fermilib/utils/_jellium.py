...
def momentum_kinetic_operator(grid, spinless=False):
    ...
    for grid_indices in grid.all_points_indices():
        momenta = momentum_vector(grid_indices, grid.length, grid.length_scale)
...
def jellium_model(grid, spinless=False, momentum_space=True):
    ...
    """
    if grid.length % 2 == 0 and grid.length & (grid.length - 1):
        raise OrbitalSpecificationError(
            'Must use an odd number or a power of 2 for momentum modes.')

    if momentum_space:
        hamiltonian = momentum_kinetic_operator(grid, spinless)
        hamiltonian += momentum_potential_operator(grid.n_dimensions,
                                                   grid.length,
                                                   grid.length_scale,
                                                   spinless)
    else:
        hamiltonian = position_kinetic_operator(grid.n_dimensions,
                                                grid.length,
                                                grid.length_scale,
                                                spinless)
        hamiltonian += position_potential_operator(grid.n_dimensions,
                                                   grid.length,
                                                   grid.length_scale,
                                                   spinless)
    return hamiltonian
